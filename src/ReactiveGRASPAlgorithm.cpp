...
ReactiveGRASPAlgorithm::ReactiveGRASPAlgorithm(const Graph &g,
    ...
    : graph(g),
      p_param(p),
      q_param(q),
      blockSize(std::max(blockSize, Config::MIN_BLOCK_SIZE)),
      totalIterations(std::max(totalIterations, Config::MIN_ITERATIONS)),
      alphaValues(alphas.empty() ? std::vector<double>{Config::DEFAULT_ALPHA} : alphas),
